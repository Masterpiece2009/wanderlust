...
import { Map as MapIcon, Navigation, Calendar, Clock, Compass } from 'lucide-react';
...
export function Roadmap() {
    ...
    return (
      <div className="space-y-12 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="h-[calc(100vh-12rem)] bg-slate-200/50 rounded-3xl animate-pulse hidden lg:block"></div>
        </div>
      </div>
    );
  }
  ...
  const mapPositions: [number, number][] = stops
    .filter((stop: any) => stop.place.location?.latitude && stop.place.location?.longitude)
    .map((stop: any) => [stop.place.location.latitude, stop.place.location.longitude]);
  ...
  return (
    <div className="space-y-12 max-w-7xl">
      ...
      >
        ...
        <p className="text-slate-500 mt-3 text-lg leading-relaxed max-w-3xl">
      ...
      {stops.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="hidden lg:block sticky top-8 h-[calc(100vh-8rem)] rounded-3xl overflow-hidden border border-slate-200 shadow-lg z-0">
            {mapPositions.length > 0 ? (
                    ...
                    <Marker key={idx} position={[loc.latitude, loc.longitude]}>
                      <Popup className="rounded-xl">
                        <div className="font-sans">
                          <div className="font-bold text-slate-900 mb-1">Stop {idx + 1}: {stop.place.name}</div>
                          <div className="text-sm text-slate-500">{loc.city}, {loc.country}</div>
                        </div>
                      </Popup>
                    </Marker>
                  );
              ...
              <div className="w-full h-full bg-slate-100 flex items-center justify-center text-slate-500 p-8 text-center">
                Map data unavailable. The recommended places do not have GPS coordinates.
              </div>
            )}
          </div>
        </div>
